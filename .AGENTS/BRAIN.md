...
* **Completed:** Initial Next.js 15 routing, basic Sanity schema setup, `.AGENTS` workspace configuration, replacing legacy `BoardMenu.tsx` with `<SpeciesCardGrid />`, refactoring the grid to a lightweight routing menu, implementing the product-species deep routing strategy (`● (SSG)`), and upgrading schemas/queries to support species-specific variant image/description overrides.
